...
export const HomePage: React.FC<HomePageProps> = ({ 
  ...
  return (
    <div className="space-y-12 pb-16">
      ...
      <GlassCard className="p-8 sm:p-12 border-cyan-500/20 overflow-hidden relative shadow-2xl">
        {/* Subtle radial ambient glows */}
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-cyan-500/10 blur-3xl rounded-full pointer-events-none" />
        <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-purple-500/10 blur-3xl rounded-full pointer-events-none" />

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center relative z-10">
          
          {/* Left Column: Brand Identity & High Contrast Headline */}
          <div className="lg:col-span-7 space-y-6">
            <GlassChip active color="cyan" icon={<Sparkles className="w-3.5 h-3.5" />}>
            ...
            <div className="space-y-3">
              ...
              <div className="text-xl sm:text-2xl font-mono text-cyan-400 font-semibold tracking-wide flex items-center gap-2">
                <span>Build.</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-6 border-t border-white/10 font-mono text-xs text-slate-300">
          ...
          <div className="lg:col-span-5 w-full">
            <GlassCard variant="subtle" className="p-6 font-mono border-white/15 space-y-5 shadow-2xl backdrop-blur-2xl">
              ...
              <div className="space-y-3 text-xs">
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/60 border border-white/10">
                  <span className="text-slate-400">Compatibility Verification</span>
                  {compatibility.overallStatus === 'compatible' ? (
                    ...
                    <span className="text-amber-400 font-bold">Check Required</span>
                  )}
                </div>

                <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/60 border border-white/10">
                  <span className="text-slate-400">Estimated Power Load</span>
                ...
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/60 border border-white/10">
                  <span className="text-slate-400">System Limiter State</span>
                  <span className="text-cyan-300 font-bold">{fpsPreview.primaryLimiter}</span>
                </div>

                <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/60 border border-white/10">
                  <span className="text-slate-400">1440p Framerate Estimate</span>
              ...
              <button
                onClick={() => onNavigate('builder')}
                className="w-full py-2.5 px-4 rounded-xl bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 text-xs font-bold transition active:scale-95 flex items-center justify-center gap-2"
              >
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-extrabold text-white font-brand">Reference Hardware Configurations</h2>
            <p className="text-xs text-slate-400">Balanced hardware configurations benchmarked for modern PC engineering workloads.</p>
          </div>
          <button 
            onClick={() => onNavigate('builder')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {featuredBuilds.map((buildItem, idx) => (
            <GlassCard key={idx} className="p-5 space-y-4 hover:border-cyan-500/40 transition-all">
              <div className="flex items-center justify-between">
                <GlassChip active color="cyan">{buildItem.badge}</GlassChip>
              ...
              <div className="space-y-2 text-xs text-slate-300 bg-slate-950/60 p-3 rounded-xl border border-white/10 font-mono">
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-slate-400">1440p FPS</span>
                  <span className="text-emerald-400 font-bold">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-white/10">
                <GlassChip active color="purple">Verified Spec</GlassChip>
                <GlassButton
                  variant="secondary"
                  size="sm"
                  onClick={() => onNavigate('builder')}
      ...
      <GlassCard className="p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {featuredComponents.map((item) => (
            ...
            >
              <img src={item.imageUrl} alt={item.name} className="w-12 h-12 object-cover rounded-xl bg-slate-900 border border-white/10" />
      ...
      <GlassCard className="p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6 border-cyan-500/30">
        <div className="flex items-start gap-4">
          <div className="p-3 rounded-xl bg-cyan-950/80 text-cyan-400 border border-cyan-500/40 shrink-0 shadow-[0_0_15px_rgba(6,182,212,0.3)]">
            <ShieldCheck className="w-6 h-6" />
          </div>
          <div>
            <h3 className="text-base font-bold text-white mb-1 font-brand">Transparent Data Provenance Guarantee</h3>
            <p className="text-xs text-slate-300 max-w-2xl leading-relaxed">
              We never fabricate benchmark numbers. Every performance metrics output clearly displays whether it originates from <strong className="text-cyan-400">⚡ Empirical Verified Test Bench Runs</strong> or <strong className="text-purple-400">📊 Physics-Based Mathematical Estimates</strong>.
            </p>
          </div>
        </div>
        <GlassButton
